...
int main() {
  ...
  //create a visitor to combine the state data
    ...
    //cout << "die" << endl; 
  //cout << theStates.getComboDemogData("MS")->getPop() << endl;
  ...
  for (auto entry : theStates.getComboDemog()) {
    ...
    totPop += entry.second->getPopD(); 
    dataYpercent.push_back(entry.second->getBelowPoverty());
    dataXpercent.push_back(entry.second->getHSup());
    /*
    if (entry.first == "MS")
    {
      cout << entry.second->getHSupCount() << endl; 
      cout << entry.second->getPopD() << endl; 
    }
    */
  }
  //cout << totPop << endl; 
  //for (auto i: dataXcount)
  //std::cout << i << ' ';
