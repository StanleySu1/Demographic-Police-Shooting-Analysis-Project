...
std::ostream& operator<<(std::ostream &out, const raceDemogData &DD) {
    ...
    if (DD.getNativeCount() > 0)
        out << " percent: " << DD.getNativePercent();
    out << " count: " << DD.getNativeCount();
    out << "\n\% Asian American";
    ...
    out << "\n\% Hispanic or Latinx";
    if (DD.getLatinoCount() > 0)
        out << " percent: " << DD.getLatinoPercent();
    out << " count: " << DD.getLatinoCount();
    ...
    out << "\ntotal Racial Demographic Count: " << DD.getCommunityCount();
    return out;
}
